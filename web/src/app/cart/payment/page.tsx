...
import { useAddresses, useAddressActions } from "@/hook/useAddress";
...
export default function PaymentPage() {
    ...
    const selectedIds = useCartStore((s: any) => s.selectedIds);
    const removeItem = useCartStore((s: any) => s.removeItem);
    const clearSelected = useCartStore((s: any) => s.clearSelected);
    ...
    const itemsTotal = selectedItems.reduce((s: number, it: any) => s + it.product.price * it.quantity, 0);
    ...
    useEffect(() => {
        if (addresses.length && selectedAddressIndex === null) setSelectedAddressIndex(0);
        if (!addresses.length) setSelectedAddressIndex(null);
    ...
    const handlePlaceOrder = async () => {
        ...
        try {
           ...
           // ✅ CASE 1: COD → thành công → thông báo → clear cart → chuyển /orders
            if (payment === "cod") {
                selectedItems.forEach((it: any) => removeItem(it.product._id));
                clearSelected();
           ...
           const message = err?.message ?? (typeof err === "string" ? err : "Đặt hàng thất bại");
           toast.error(message);
        }
    ...
    useEffect(() => {
        ...
        setEstimatedDelivery(`${deliveryETA} — khoảng ${now.toLocaleDateString()} đến ${end.toLocaleDateString()}`);
